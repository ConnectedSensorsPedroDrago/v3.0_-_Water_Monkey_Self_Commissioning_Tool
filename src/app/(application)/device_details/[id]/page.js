...
const WaterMonkeyDeviceDetails = ({ params }) => {
    ...
    fetch(`/api/devices/water-monkey/get-device?id=~${params.id}`)
      ...
      .then(data => {
        if(data.status === "error"){
          ...
          properties.commission_stage && setCommStage(JSON.parse(properties.commission_stage))
        }
      })
  ...
  return (
    <div className='container-pages h-fit'>
      ...
      }
      {
        ...
        <CommToolTop 
        title={""} 
        back={(commStage && (commStage.stage === "commissioned" || commStage.stage === "recommission" || commStage.stage === "recommission_failed") ) ? `/dashboard/${id}` : `/comm-tool/step-3/${params.id}`} 
        address={address && address}
      />
      }
      <h1 className="text-[1.5rem] lg:text-[3.25rem] font-bold text-center text-purple mt-[2rem] mb-[2rem]">
        Property and Meter Details
      </h1>
      {message &&
        <Message 
          message={message}
      ...
      {
        commStage && (commStage.stage === 'none' || commStage.stage === 'first reading' || commStage.stage === 'second reading') &&
          <WarningSign 
...
export default WaterMonkeyDeviceDetails
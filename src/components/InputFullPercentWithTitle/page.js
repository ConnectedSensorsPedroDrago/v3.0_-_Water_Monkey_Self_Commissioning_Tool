...
const InputFullPercentWithTitle = ({name, setter, placeholder, type, disabled}) => {
  ...
  return (
    <div className="flex flex-col justify-start w-full">
        ...
        <input 
          className={`min-w-[100%] bg-white input-small ${type === ("file" || "datetime-local") ? `disabled:text-grey` : disabled === true && `disabled:text-grey disabled: disabled:bg-white font-semibold`}`}
          onChange={(e)=> {
            if(type === 'file'){
